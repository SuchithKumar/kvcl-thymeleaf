...
public class ImageFilter {
	...
	static Logger logger = LoggerFactory.getLogger("ImageFilter.class");
	
	public static void main(String[] args) {
		
		
		File file = new File("/Users/suchithkumar/git/kvcl-thymeleaf/src/main/resources/static/images/scanner.jpg");
...
//			img = detectEdges(img);
	...
	public static byte[] resize (MultipartFile file, int newHeight, String type) {	
		logger.info("original file size : {}",file.getSize());
		BufferedImage img = null;
		try { img = ImageIO.read(file.getInputStream()); } 
		catch (IOException e) { e.printStackTrace(System.out); }
		
		System.out.println("  Scaling image.");
		double scaleFactor = (double) newHeight/img.getHeight();
		BufferedImage scaledImg = new BufferedImage(
			(int)(scaleFactor*img.getWidth()), newHeight, BufferedImage.SCALE_FAST);
		AffineTransform at = new AffineTransform();
		at.scale(scaleFactor, scaleFactor);
		AffineTransformOp scaleOp = new AffineTransformOp(at, AffineTransformOp.TYPE_BILINEAR);	
		var output = scaleOp.filter(img, scaledImg);
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		byte[] imageInByte = null;
		try {
			ImageIO.write(output, type, baos);
			baos.flush();
			imageInByte = baos.toByteArray(); // you have the data in byte array
			baos.close();			
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		logger.info("resized file size : {}",file.getSize());
		
		logger.info("original is greater than resized? - {}",file.getSize()>imageInByte.length);
		return imageInByte;
	}
